...
local POS = {
        L1 = Vector3.new(-476.85,-6.59,94.91), L2 = Vector3.new(-485.55,-4.53,100.61),
        L3 = Vector3.new(-475.60,-6.59,92.80), L4 = Vector3.new(-475.26,-6.57,21.54),
        R1 = Vector3.new(-475.77,-6.57,26.76), R2 = Vector3.new(-485.85,-4.48,20.13),
        R3 = Vector3.new(-475.83,-6.59,26.54), R4 = Vector3.new(-476.17,-6.09,97.73),
}

-- Waypoint speed config: waypoints 1-2 use GoingSpeed, waypoints 3-4 use StealSpeed
local WaypointSpeeds = { GoingSpeed = 57, StealSpeed = 29 }
local leftWaypoints  = { POS.L1, POS.L2, POS.L3, POS.L4 }
local rightWaypoints = { POS.R1, POS.R2, POS.R3, POS.R4 }
...
local function createMobilePanel()
        ...
        local dropBRSetActive = makeSeparateBtn("BtnDropBR", "DROP\nBR",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[1]), "VexulBtn_dropbr.txt",
        ...
        local autoLeftSetActive = makeSeparateBtn("BtnAutoLeft", "AUTO\nLEFT",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[1]), "VexulBtn_autoleft.txt",
                function(setActive)
        ...
        local autoBatSetActive = makeSeparateBtn("BtnAutoBat", "BAT\nAIMBOT",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[2]), "VexulBtn_autobat.txt",
                function(setActive)
        ...
        local autoRightSetActive = makeSeparateBtn("BtnAutoRight", "AUTO\nRIGHT",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[2]), "VexulBtn_autoright.txt",
                function(setActive)
        ...
        local tpDownSetActive = makeSeparateBtn("BtnTpDown", "TP\nDOWN",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[3]), "VexulBtn_tpdown.txt",
                function(setActive, currentActive)
                        tpToGround()
        ...
        local carrySpeedSetActive = makeSeparateBtn("BtnCarrySpd", "CARRY\nSPD",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[3]), "VexulBtn_carryspd.txt",
                function(setActive)
                        toggleSpeedType()
        ...
        local laggerCarrySetActive = makeSeparateBtn("BtnLaggerCarry", "LAGGER\nCARRY",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[4]), "VexulBtn_laggercarry.txt",
                function(setActive)
                        toggleLaggerCarry()
        ...
        local laggerSetActive = makeSeparateBtn("BtnLaggerMode", "LAGGER\nMODE",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[4]), "VexulBtn_laggermode.txt",
                function(setActive)
                        toggleLagger()
...
-- ========== PREMIUM BLACK BACKGROUND WITH FALLING STARS ==========
local bgFrame = Instance.new("Frame", gui)
bgFrame.Name = "PremiumBackground"
...
local function createStar(index, randomizeY)
        local size = math.random(1, 3)
        local star = Instance.new("Frame", bgFrame)
        ...
        star.BackgroundTransparency = math.random(0, 40) / 100
        star.Size = UDim2.new(0, size, 0, size + math.random(2, 6))
        star.ZIndex = -9
        Instance.new("UICorner", star).CornerRadius = UDim.new(1, 0)

        local xPos = math.random(0, 1000) / 1000
        local yStart = randomizeY and (math.random(-100, 100) / 100) or -0.05
        star.Position = UDim2.new(xPos, 0, yStart, 0)

        local speed = math.random(40, 160) / 100  -- fall speed multiplier
        local drift = (math.random(-8, 8)) / 1000  -- slight horizontal drift
...
RunService.RenderStepped:Connect(function(dt)
        for i, s in ipairs(starData) do
                s.y = s.y + s.speed * dt * 0.18
                s.x = s.x + s.drift * dt
                if s.y > 1.05 then
                        -- Recycle star at top
                        s.y = -0.08
                        s.x = math.random(0, 1000) / 1000
                        s.frame.BackgroundColor3 = Color3.fromRGB(
                                math.random(200, 255),
                                math.random(200, 255),
                                math.random(210, 255)
                        )
                        s.frame.BackgroundTransparency = math.random(0, 40) / 100
                end
...
local function saveStealBarPosition()
    ...
    local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
    pcall(function() writefile("VexulStealBarPos.txt", str) end)
end

local function loadStealBarPosition()
    local savedPos = nil
    pcall(function() savedPos = readfile("VexulStealBarPos.txt") end)
    if savedPos and savedPos ~= "" then
        local parts = {}
        for part in string.gmatch(savedPos, "[^,]+") do table.insert(parts, part) end
        if #parts >= 4 then
...
footerLbl.Text="vexul hub  ·  v1.0"; footerLbl.TextColor3=Color3.fromRGB(50,50,50)
...
startAutoLeft = function()
        ...
        Conns.autoLeft = RunService.Heartbeat:Connect(function()
                ...
                -- Phases 1-2 use GoingSpeed, phases 3-4 use StealSpeed
                local spd = (State.autoLeftPhase <= 2) and WaypointSpeeds.GoingSpeed or WaypointSpeeds.StealSpeed
                local waypoints = leftWaypoints
                local tgtPos = waypoints[State.autoLeftPhase]
                if not tgtPos then return end
                local tgt = Vector3.new(tgtPos.X, root.Position.Y, tgtPos.Z)
                if (tgt - root.Position).Magnitude < 2.5 then
                        State.autoLeftPhase = State.autoLeftPhase + 1
                        if State.autoLeftPhase > #waypoints then
...
startAutoRight = function()
        ...
        Conns.autoRight = RunService.Heartbeat:Connect(function()
                ...
                if not root or not hum2 then return end
                -- Phases 1-2 use GoingSpeed, phases 3-4 use StealSpeed
                local spd = (State.autoRightPhase <= 2) and WaypointSpeeds.GoingSpeed or WaypointSpeeds.StealSpeed
                local waypoints = rightWaypoints
                local tgtPos = waypoints[State.autoRightPhase]
                if not tgtPos then return end
                local tgt = Vector3.new(tgtPos.X, root.Position.Y, tgtPos.Z)
                if (tgt - root.Position).Magnitude < 2.5 then
                        State.autoRightPhase = State.autoRightPhase + 1
                        if State.autoRightPhase > #waypoints then
                                hum2:Move(Vector3.zero, false)
                                root.AssemblyLinearVelocity = Vector3.new(0, root.AssemblyLinearVelocity.Y, 0)
